...
Note: In the current version of this package, it is now possible to UPC normalize data from any expression-profiling platform. The documentation describes the \Rfunction{UPC\_Generic} and \Rfunction{UPC\_Generic\_ExpressionSet} functions, which enable this functionality.
